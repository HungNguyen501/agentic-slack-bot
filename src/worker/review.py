...
from connectors import databricks, github, postgres
...
def handle_review_decision(
    ...
    row = postgres.get_access_request(bot.bot_id, request_id, thread_ts)
    if not row:
    ...
    if decision == "reject":
        postgres.update_access_request_status(row["id"], "rejected")
        return f"Request `{request_id}` rejected by <@{user_id}>."

    return _approve(bot, row, channel, user_id)


def _approve(bot: BotConfig, row: dict, channel: str, approver_id: str) -> str:
    ...
    category = postgres.get_access_request_category(bot.bot_id, row["request_type"])
    if not postgres.channel_authorized(category, row["channel"]):
        return (
    ...
    postgres.update_access_request_status(
        row["id"],
        "approved",
        pr_url=pr_url,
