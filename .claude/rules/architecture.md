...
3. **Cache check** — `connectors/db/service_principal_secrets.py::get` looks up a cached row (keyed by `bot_id` + `email`, the bare address). Databricks only ever returns a secret's plaintext once, at creation, so this table stores it encrypted (`common/crypto.py`, Fernet, key from `Configs.SECRET_ENCRYPTION_KEY`) to make "return the existing secret if still valid" possible at all. If the cached row is `status == "ACTIVE"` and unexpired, it's decrypted and reused — no new Databricks secret is minted (Databricks caps active secrets per service principal).
